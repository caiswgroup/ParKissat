...
KissatBonusSolver::diversify(int id)
{
   if (Parameters::getBoolParam("bump")) {
      if (id && solver->initshuffle == -1) solver->bump_one=rand()%solver->max_var+1;
   }
...
KissatBonusSolver::setParameter(parameter p)
{
   ...
   // printf("c\t\tchrono=%d stable=%d target=%d phase=%d restartmarin=%d\n",

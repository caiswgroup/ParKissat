...
{
  assert(!solver->stable && solver->heuristic == 0);
...
{
  assert(solver->stable || solver->heuristic == 1);
  const unsigned idx = IDX(lit);
    ...
    if (!solver->heuristic) {
      ...
      for (unsigned i = j; i != old_size; i++)
      {
        ...
        else
        {
          ...
          if (age > 0)
          {
            double reward_chb = 1.0 * solver->conflicted_chb[idx] / age;
            double old_score = kissat_get_heap_score(&solver->scores_chb, idx);
            double new_score = solver->step_chb * reward_chb + (1 - solver->step_chb) * old_score;
            LOG("new score[%u] = %g vs %g",
                v, new_score, old_score);
            kissat_update_heap(solver, &solver->scores_chb, idx, new_score);
          }

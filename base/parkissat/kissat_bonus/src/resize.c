...
kissat_increase_size (kissat * solver, unsigned new_size)
{
  ...
  if(solver->heuristic==1) {
    ...
    CREALLOC_VARIABLE_INDEXED (unsigned,canceled);
    for (int i = 0; i < new_size; i++) solver->picked[i] = solver->canceled[i] = solver->conflicted_chb[i] = 0;
  }
...
kissat_decrease_size (kissat * solver)
{
  ...
  if(solver->heuristic==1) {
     ...
     NREALLOC_VARIABLE_INDEXED (unsigned, canceled);
  }
